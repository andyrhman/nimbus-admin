...
const UserChart = () => {
  const [series, setSeries] = useState([{ name: "Perencanaan Manual", data: [] }]);
  ...
  const fetchData = async (time) => {
    try {
      const response = await axios.get(`admin/po-chart?time=${time}`);
      ...
      setCategories(dates);
      setSeries([{ name: "Total Perencanaan Otomatis", data: counts }]);
    } catch (error) {
        ...
        <div className="flex w-full flex-wrap gap-3 sm:gap-5">
          <div className="flex min-w-47.5">
            ...
            <div className="w-full">
              <p className="font-semibold text-primary">Chart Perencanaan Otomatis</p>
            </div>
          </div>
        </div>
