...
const UserChart = () => {
  ...
  const fetchData = async (time) => {
    try {
      const response = await axios.get(`admin/pm-chart?time=${time}`);
      ...
      setCategories(dates);
      setSeries([{ name: "Total Perencanaan Manual", data: counts }]);
    } catch (error) {
        ...
        <div className="flex w-full flex-wrap gap-3 sm:gap-5">
          <div className="flex min-w-47.5">
            ...
            <div className="w-full">
              <p className="font-semibold text-primary">Chart Perencanaan Manual</p>
            </div>
          </div>
        </div>
