...
const Users = () => {
    const pageTitle = `Users | ${process.env.siteTitle}`;
        ...
        (
            async () => {
                try {
                    ...
                    const { data: user } = await axios.get(`admin/users?${arr.join('&')}`);
                    setUsers(user);
                    setPage(0);
                } catch (error) {
                                        ...
                                        {users.slice(page * perPage, (page + 1) * perPage).map((user, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                                    {user.is_user === false ? (
                                                        <p
                                                            className="inline-flex rounded-full bg-opacity-10 px-3 py-1 text-sm font-medium bg-danger text-danger"
                                                        >
                                                            Admin
                                                        </p>
                                                    ) : (
