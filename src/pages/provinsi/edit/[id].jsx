...
const EditProvinsi = () => {
    ...
    useEffect(() => {
        if (id) {
            (
                async () => {
                    try {
                        const { data } = await axios.get(`admin/provinsi/${id}`);
                        setNama(data.nama);
    ...
    const updateProvinsi = async (e) => {
        ...
        try {
            await axios.put(`admin/provinsi/${id}`, {
                nama,
                thumbnail
            });
        ...
        <Layout>
            ...
            <DefaultLayout>
                <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <Link href="/provinsi" className="flex items-center space-x-2">
                            ...
                            <div className="flex flex-col gap-5.5 p-6.5">
                                ...
                                <form>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            value={thumbnail}
                                            onChange={(e) => setThumbnail(sanitizeHtml(e.target.value))}
                                            type="text"
                                            className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                                        />
